...
/**
  ...
  * This part is mainly for feature creation. It basically replicates the original spot-ml feature creation for the LDA implementation
  */
object supervisedFeatureCreation {
  ...
  def run(spark: SparkSession, DF1: DataFrame): DataFrame ={
    ...
    val DF3 = DF2.select("label","host", "fulluri", "resconttype","useragent","respcode" ,"reqmethod","uid").rdd.map {
      case Row (   label: Int,
      host: String,
      fulluri: String,
      resconttype: String,
      ...
      "label","domain","fulluri", "resconttype","useragent","respcode", "reqmethod","uid")
    //DF3.show()

    /**In this part the fixed bins are computer */
    val DF4rdd=  DF3.select("label","domain","fulluri", "resconttype","useragent","respcode" ,"reqmethod", "uid").rdd.map {
      case Row (     label: Int,
      domain: String,
      fulluri: String,
      resconttype: String,
    ...
    val DF4 = DF4rdd.toDF( "label","alexa", "entropy","resconttype", "useragent","uriLen","respcode","reqmethod","uid")

    //DF4.show()

    //DF4.printSchema()
    /** In this part we check of empty strings */
    val DF5 =  DF4.select("label","alexa", "entropy","resconttype", "useragent","uriLen","respcode","reqmethod","uid").rdd.map {
      case Row (
      label: Int,
      alexa: Int,
      entropy: Int,
